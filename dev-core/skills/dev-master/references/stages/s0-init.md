...
1. 确认项目根目录、子项目划分与命名——**子项目一律建在 `dev/code/` 下**（`dev/code/admin/`、
   `dev/code/mobile/`、`dev/code/server/` 等；单端项目直接 `dev/code/src/`）
2. 建骨架：目录结构、包管理、lint/format 配置、`.env.example`、`.gitignore`
3. 写 `README-DEV.md`：目录约定、命名规范、路由方式、Mock 约定、启动命令
4. 建 `dev/` 落盘骨架：`dev/{SRS,design,plan,test,reports,release,code}/`（研发链所有产出的唯一去处，
   阶段 1 的 SRS 落 `dev/SRS/`，阶段 7 的代码落 `dev/code/`）。仓库级基建（compose / CI / hooks /
   scripts / 部署文档）留仓库根
...
- [ ] `dev/` 骨架已建（`SRS/ design/ plan/ test/ reports/ release/ code/`），研发链产出一律落这儿；
